...
`;
  ...
  var PredictiveSearchInput = class extends HTMLInputElement {
    connectedCallback() {
      this.addEventListener("click", () => document.getElementById(this.getAttribute("aria-controls")).open = false);
    }
  };
  ...
  var FacetFilters = class extends DrawerContent {
    ...
    _onFilterChanged() {
      ...
      const activeFilters = Array.from(formData.entries()).filter(([key, value]) => key.startsWith("filter.") && value);

      let searchInputB2B = document.getElementById("collection-search-input");
      if (searchInputB2B) {
        if (activeFilters.length > 0) {
          // console.log("🔴 V3: Filtros activos, bloquean buscador");
          searchInputB2B.disabled = true;
          searchInputB2B.classList.add("input-disabled");
        } else {
